...
export type DepartmentShareOverTime = {
  years: string[];
  lines: Array<{ name: string; data: number[] }>;
};

export function buildDepartmentShareOverTime(data: DashboardData): DepartmentShareOverTime {
  const years = data.table5.years;
  ...
  // Convert and sort by latest year share (descending)
  const lines = Array.from(deptMap.values())
    .map((dept) => ({
